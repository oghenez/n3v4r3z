...
<div id="contentAll" class="f-l">
	...
	<table class="tblListados corner-all8 a-c" style="font-size:0.7em;">
		<tr class="header btn-gray">
			<td <?php echo ($lista['historial'])?'':'style="display:none;"' ?> class="header-sema">P. Puntualidad </td>
			...
			<td <?php echo ($lista['historial'])?'':'style="display:none;"' ?> class="header-sema-total">Premio Puntualidad</td>
		...
		foreach($lista['empleados'] as $emp){
			...
			$total_semanal += ($lista['historial'])?$emp->salario*$emp->dias_trabajados:$emp->salario*7;
			...
			?>
				<tr id="<?php echo str_replace('.', '-', $emp->id_empleado)?>">
					<td id="semana" <?php echo ($lista['historial'])?'':'style="display:none;"' ?>> 
					...
					<td>
							...
							<input type="hidden" name="fsueldo_semanal[]" value="<?php echo floatval($emp->salario*7) ?>" id="fsueldo_semanal" />

							<input type="hidden" name="fpremio_puntualidad[]" value="0" id="fpremio_puntualidad" />
							...
							<input type="hidden" name="ftotal_pagar[]" value="<?php echo floatval($emp->salario*7) ?>" id="ftotal_pagar" />
					...
					<td>
							<input type="number" name="fdias_trabajados[]" 
										value="<?php echo ($lista['historial'])?$emp->dias_trabajados:'7' ?>" id="fdias_trabajados" class="a-c vpos-int" size="1" min="0" max="7" <?php echo ($lista['historial'])?'disabled':''?>/>
					</td>
					<td><?php echo String::formatoNumero($emp->salario)?> </td>
					<td id="fsueldo_semanal">
						<?php echo String::formatoNumero(($lista['historial'])?$emp->sueldo_semanal:$emp->salario * 7) ?> 
					</td>
					<td id="total_premio_puntialidad" style="<?php echo ($lista['historial'])?'':'display: none;'?> background: rgba(108, 182, 255, .2);">
						<?php echo ($lista['historial'])?String::formatoNumero($emp->premio_puntualidad):''; ?>
					</td>
		...
		<?php } $ttotal_pagar = $total_semanal + $ttotal_puntualidad + $ttotal_eficiencia + $ttotal_vacaciones + $ttotal_aguinaldo;?>
				<tr style="background-color:#ccc;font-weight: bold;" id="ttotales">
					<td colspan="<?php echo ($lista['historial'])?'10':'7'?>">
						<input type="hidden" name="ttotal_salario" value="<?php echo $total_salario ?>" id="ttotal_salario" />
						<input type="hidden" name="ttotal_semanal" value="<?php echo $total_salario*7 ?>" id="ttotal_semanal" />
						<input type="hidden" name="ttotal_pagar" value="<?php echo $total_salario*7 ?>" id="ttotal_pagar" />
					</td>
					...
					<td id="ttotal_puntualidad" <?php echo ($lista['historial'])?'':'style="display:none;"'?> ><?php echo String::formatoNumero(($lista['historial'])?$ttotal_puntualidad:0); ?></td>
					...
					<td id="ttotal_pagar"><?php echo String::formatoNumero(($lista['historial'])?$ttotal_pagar:$total_semanal); ?></td>
				</tr>
			</table>
	</form>
</div>
